...
//   getAllWorkIds2() {
//     const p = new Promise(async (resolve, reject) => resolve(await prisma.work.findMany({
//       select: {id: true}
//     })))
//     return p.map()
//   }
  
//   async getNumWorks(): Promise<number> {
//     return await prisma.work.count()
//   }
  
  // // async getRandWork(): Promise<Work> {
  // //   const total: number = await this.getNumWorks()
  // //   const skip: number = getRandNum(total)
  ...
  // // async getRandWorkIds(n: number, excludeId: string|null = null): Promise<string[]> {
  // //   const ids: string[] = await this.getAllWorkIds()
  // //   if (excludeId !== null && ids.includes(excludeId)) {
  // //     ids.splice(ids.indexOf(excludeId), 1)
  // //   }
  ...
  // async getMultipleWorkPropVars(ids: string[], property: string): Promise<any> {
  //   const selectDict: {[key: string]: boolean} = {}
  //   selectDict[property] = true
  //   return (await prisma.work.findMany({
  //     where: {id: {in: ids}},
  ...
  // async getWorkPropVars(property: string): Promise<string[]> {
  //   const selectDict: {[key: string]: boolean} = {}
  //   selectDict[property] = true
  //   return (await prisma.work.findMany({
  //     select: selectDict
  ...
  // async getRandWorkPropVarsExcept(property: string, n: number, exception: string|null = null): Promise<string[]> {
  //   const variations: string[] = await this.getWorkPropVars(property)
  //   const uniqueVariations = removeDuplicates<string>(variations)
  ...
  // async getRandomABCDQuestionOld() {
  //   const randomWorkIds: string[] = await this.getRandWorkIds(4)
  ...
  //   const possibleAnswers: string[] = await this.getMultipleWorkPropVars(randomWorkIds, value)
  
  //   // @ts-ignore
  //   const mainWorkId: string = randomWorkIds.pop()
  //   const mainWork: Work = await this.getWorkById(mainWorkId)
  ...
  //   const randomWork: Work = await this.getRandWork()
  ...
  //   const answerOptions = await this.getRandWorkPropVarsExcept(property, 3, correctAnswer)
  //   answerOptions.push(correctAnswer)
  //   shuffleArray(answerOptions)
